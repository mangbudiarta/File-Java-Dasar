...
public class InsertionSort {
	...
	public static void main(String[] args) {
		...
		insertionMenurun(L, N);
		input.close();
	}

	// code prosedur untuk mengurutkan array menaik
	public static void insertionMenaik(byte L[], byte N) {
		for (byte i = 1; i < N; i++) {
			byte x = L[i];
			int j = i - 1;
	...
	public static void insertionMenurun(byte L[], byte N) {
		for (byte i = 1; i < N; i++) {
			byte x = L[i];
			int j = i - 1;
			boolean ketemu = false;
